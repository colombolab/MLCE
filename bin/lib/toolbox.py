...
def READPDBFILE(pdbfilename):
    ...
    for line in INPUT.readlines():
        ...
        if atom[0] != 'ATOM':
            ...
            atom.append(''.join(list(temp[6:11])).replace(
                ' ', ''))  # Atom serial number
            atom.append(''.join(list(temp[12:16])).replace(
                ' ', ''))  # Atom name
            atom.append(''.join(list(temp[16])).replace(' ', ''))  # AltLoc
            atom.append(''.join(list(temp[17:20])).replace(
                ' ', ''))  # Residue name
            atom.append(''.join(list(temp[21])).replace(
                ' ', ''))  # Chain Identifier
            atom.append(''.join(list(temp[22:27])).replace(
                ' ', ''))  # Sequence number
            atom.append(''.join(list(temp[27])).replace(
                ' ', ''))  # iCode - OVER 1000aa fix
            atom.append(''.join(list(temp[30:38])).replace(
                ' ', ''))  # x coordinate
            atom.append(''.join(list(temp[38:46])).replace(
                ' ', ''))  # y coordinate
            atom.append(''.join(list(temp[46:54])).replace(
                ' ', ''))  # z coordinate
            atom.append(''.join(list(temp[54:60])).replace(
                ' ', ''))  # Occupancy
            atom.append(''.join(list(temp[60:66])).replace(
                ' ', ''))  # Temperature factor
            atom.append(''.join(list(temp[72:76])).replace(' ', ''))  # SegID
            atom.append(''.join(list(temp[76:78])).replace(
                ' ', ''))  # Element symbol
...
def DISTANCE_MATRIX(pdbfileaccess, topolcontrol, radius, structure):
    ...
    for i in range(0, length):
        for h in range(0, length):
            if i == h:
                ...
                distance_CB_CB[i, h] = math.sqrt(
                    x_distance[i, h] * x_distance[i, h] + y_distance[i, h] * y_distance[i, h] + z_distance[i, h] * z_distance[i, h])
                if distance_CB_CB[i, h] <= radius:
                    distance_matrix[i, h] = 1
                else:
                    distance_matrix[i, h] = 0
    ...
    if topolcontrol == 1:
        ...
        for i in range(0, length):
            for h in range(0, length):
                OUTPUT.write(str(i+1)+"\t"+str(h+1)+"\t" +
                             str(int(distance_matrix[i, h]))+"\n")
            OUTPUT.write("\n")
        OUTPUT.close()
...
def ENERGY_MATRIX(eigenvect, eigenval):
    ...
    # component of energy matrix
    energy_component = np.zeros(shape=(length, length))
    ...
    for i in range(0, length):
        for j in range(0, length):
            energy_component[i, j] = eigenvector[i] * \
                eigenvector[j] * eigenval[0]
...
def MLCE(protein_length, threshold, distance_matrix, energy, mlceprint):
    counter = 0
    # Matrix of local coupling energies
    mlce = np.zeros(shape=(protein_length, protein_length))
    ...
    for line in approx_matrix:
        if line[2] != 0. and counter > limit:
            epitopes_raw.append(line)
        elif line[2] != 0. and counter <= limit:
            counter += 1
    ...
    for i in range(0, length):
        for j in range(0, length):
            if distance_matrix[epitopes_raw[i]][epitopes_raw[j]] == 1:
                if patch_strings[h] is None:
                    patch_strings[h] = [epitopes_raw[j]]
                else:
                    patch_strings[h].append(epitopes_raw[j])

        if patch_strings[h] is None:
            patch_strings[h] = [epitopes_raw[j]]
        else:
            patch_strings[h].insert(0, epitopes_raw[i])
        h += 1
...
def WRITE(pdbfileaccess, renumbering, span, scored, structure,
    ...
    # list of string with 1-letter sequence of patches
    code = [None]*len(scored)
...
""" % (version, sversion, ssversion))
    ...
    for i in range(0, length):
        ...
        for res in scored[i]:
            index = list(zip(*info))[0].index(res)
            LOG.write(str(info[index][0]) + "\t" + info[index]
                      [1] + "\t" + info[index][2] + "\n")
            if code[i] is None:
                code[i] = [info[index][2]]
            else:
