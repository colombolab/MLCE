...
# Modules
import sys,os,subprocess,shutil		# Shell interaction
...
def main():
	...
	'gnuplot_bin'   : GP_path,
        ...
        'tleap_bin'        : '/opt/share/sw/gcc-7.3.1/amber18/bin/tleap',
        'sander_bin'    : '/opt/share/sw/gcc-7.3.1/amber18/bin/sander',
        'mmpbsa_bin'    : '/opt/share/sw/gcc-7.3.1/amber18/bin/mm_pbsa.pl',
        'mmpbsapy_bin'    : '/opt/share/sw/gcc-7.3.1/amber18/bin/MMPBSA.py',
        'sandermpi_bin'    : '/opt/share/sw/gcc-7.3.1/amber18/bin/sander.MPI',
	...
	if mode == 'b' and 'res_range' in vars():
		Header()
		Error('With [--range] option enabled it is not possible to start BEPPE.\n')
	...
	for element in address:
		if mpi == 0 and (element <> 'mpirun_bin' or element <> 'sandermpi_bin'):
			...
			if not os.path.exists(address[element]):
				Error('Path ' + address[element] + ' for ' + element + ' not found\n')
	...
	if mode == 's' or mode == 'b':
		...
		# Prepare AMBER input
		resnumber = AmberConfig(currentpdb, folder, logfile, address, dielectric)

		if coevo == 0 and ext_mat == 0:
			# Run minimization and MM_GBSA
			AmberJobs(currentpdb,folder,logfile,address,python,resnumber,nproc)
		elif coevo == 1:
			# Check coevolutionary matrix existance
			if not os.path.exists('../' + coevo_path):
				Error('Path ' + coevo_path + ' for coevolutionary matrix not found.')
			...
			if not os.path.exists('../' + ext_mat_path):
				Error('Path ' + ext_mat_path + ' for coevolutionary matrix not found.')
			shutil.copyfile('../' + ext_mat_path,'ele-vdw.dat')

		# Diagonalize energy matrix
		Diagonalize(currentpdb,folder,logfile,resnumber,'ele-vdw.dat',address)

		# Produce energy matrix and (optionally) energy profile
		profile_1d, profile_2d = Automan(folder, logfile, 'EIGENVECT.txt', 'EIGENVAL.txt', mode, domain)
		...
		if mode == 'b':
			os.chdir(folder)
			Beppe(folder,logfile,beppe,beppe_bin,beppepdb,topology,mlceprint)
		Fine(0)
	...
	if mode == 'm' or mode == 'c':
		...
		if mode == 'm':
			...
			ClustersEnergy(workdir,frame_number,resnumber,multienergy,cluster_length)
		# Set the first frame as reference structure
		currentpdb = workdir + '/FRAME_1/snapshot.AMBER.pdb'

		# Diagonalize the average matrix		
		Diagonalize(currentpdb,workdir,logfile,resnumber,'ele-vdw_average.dat',address)
		
		profile_1d, profile_2d = Automan(workdir, logfile, 'EIGENVECT.txt', 'EIGENVAL.txt', mode, domain)
...
def AmberPDB(pdb, workdir, logfile):
	...
	print "\nI-", date('%Y-%m-%d %H:%M:%S'), "converting pdb format for AMBER...\n"
	
	logfile.write("\n\n*** AMBER PDB CONVERSION %s" % str(date('%Y-%m-%d %H:%M:%S')) );
		...
		if line[:4] == 'ATOM':
			if line[21] == ' ':
				pdb_content.append([line[0:5],          # [0] Datatype
									line[5:11],         # [1] Atom serial number
									line[11],           # [2] (NOT DEFINED IN STANDARD)
									line[12:16],		# [3] Atom name
									line[16],		    # [4] Alternate location
									line[17:20],		# [5] Residue name
									line[20],		    # [6] (NOT DEFINED IN STANDARD)
									chr(chainid),		# [7] Chain ID
									line[22:26],		# [8] Residue seq. number
									line[26],		    # [9] Code for residues insertion
									line[27:30],		# [10]
									line[30:38],		# [11] x coordinate
									line[38:46],		# [12] y coordinate
									line[46:54],		# [13] z coordinate
									line[54:60],		# [14] Occupancy volume
									line[60:66],		# [15] T-factor
	...
	inc_atom = 1; inc_res = 1
	current_chain = pdb_noH[0][7]
	current_res = pdb_noH[0][8]
	prev_resn = 1; prev_ins = pdb_noH[0][9]
...
def AmberConfig(pdb, folder, logfile, address, dielectric):
	...
	print "I- %s configuring AMBER input files..." % str(date('%Y-%m-%d %H:%M:%S'))
...
''' % pdb
	...
	with open(folder + '/min.in','w') as f:
		f.write('%s' % minfile.replace("RES start_number end_number", "RES 1 %d" % tot_res))
	...
	# Editing mm_pbsa.in (no dielectric change)
	    ...
	    with open(folder + '/mm_pbsa.in','w') as f:
		    f.write('%s' % mmpbsafile.replace("start-end", "1-%d" % tot_res).replace("_FILL_",str(dielectric)))
...
def AmberJobs(pdb,folder,logfile,address,python,resnumber,nproc):
	...
	# Perl routine 
	if python == 0:
		print "\t%s MM-GBSA with energy decomposition" % str(date('%Y-%m-%d %H:%M:%S'))

		logfile.write("\n\n*** MM-GBSA RUN %s\n" % str(date('%Y-%m-%d %H:%M:%S')) )

		# MM-GBSA
		subprocess.call(["cp", folder + '/prot-min.restrt', folder + '/prot-min_rec.crd.1'])
	...
	# Python routine
		...
		print "\t%s MM-GBSA with energy decomposition (python)" % str(date('%Y-%m-%d %H:%M:%S'))
		...
		for line in in_content:
			...
			if record[0] == 'Backbone Energy Decomposition:': 
				...
				en_matrix[int(record[0][3:]) - 1,int(record[1][3:]) - 1] += \
					float("%.2f" % float(record[5])) + float("%.2f" % float(record[8])) + float("%.2f" % float(record[11])) + float("%.2f" % float(record[14]))
			else:
				pass
...
def Automan(folder, logfile, evecfile, evalfile, mode, domain):
	...
	print "\nI- %s energy eigendecomposition..." % str(date('%Y-%m-%d %H:%M:%S'))
	...
	if domain == 1:
		...
		for col_id in range(0,resnumber):
			boolcolumn = np.array([ x > np.median(np.abs(evecs[:,col_id])) for x in np.abs(evecs[:,col_id])])
			try:
				# Add new eigenvector
		...
		while len(remaining)>0:	
			...
			if np.sum(newcoverage >= 3) > resnumber/2.:
				print "\t%s More than 50%% of the sequence is covered by at least 3 eigenvectors!" % str(date('%Y-%m-%d %H:%M:%S'))
				break
			...
			if newinfo/float(resnumber) > 0.01:
				coverage = np.logical_or(coverage, boolmatrix[:,remaining[sel_id]])
				newcoverage += axorb
				print "\t%s New eigenvector selected!" % str(date('%Y-%m-%d %H:%M:%S'))
...
def EneMatrix(folder,resnumber,eigenvectors,eigenvalues,mode,sel_eig):
	...
	for index in sel_eig:
		for i in range(0,resnumber):
			for j in range(0,resnumber):
				enematrix[i,j] += eigenvalues[index] * eigenvectors[i,index] * eigenvectors[j,index]
...
def Matrice(folder,logfile,address,res_range,mode):
    ...
    ave = np.sum(content[:,2])/(float((res_range[1]-res_range[0])*(res_range[1]-res_range[0])))
...
================================================================================
		...
		Renewed and Extended BEppe Layer Over Trajectories
				
				    ver. 1.2.3

--------------------------------------------------------------------------------
    Copyright (c) 2011-2019, Riccardo Capelli, Claudio Peri, Dario Corrada.
