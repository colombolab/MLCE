...
def main():
    ...
    address = {
        ...
        'tleap_bin': '/data1/sw/Amber/amber18/bin/tleap',
        'sander_bin': '/data1/sw/Amber/amber18/bin/sander',
        'mmpbsa_bin': '/data1/sw/Amber/amber18/bin/mm_pbsa.pl',
        'mmpbsapy_bin': '/data1/sw/Amber/amber18/bin/MMPBSA.py',
        #'mmpbsapympi_bin': '/home/administrator/amber18/bin/MMPBSA.py.MPI',
        'sandermpi_bin': '/data1/sw/Amber/amber18/bin/sander.MPI',
        'mpirun_bin': '/data1/sw/openmpi/4.0.3/bin/mpirun'
    }
    REB_log = '/REBELOT.log'
    ...
    if mode == 's' or mode == 'b':
        ...
        profile_1d, profile_2d = Automan(
            folder, logfile, 'EIGENVECT.txt', 'EIGENVAL.txt', mode, domain, resnumber)
    ...
    if mode == 'm' or mode == 'c':
        ...
        profile_1d, profile_2d = Automan(
            workdir, logfile, 'EIGENVECT.txt', 'EIGENVAL.txt', mode, domain, resnumber)
...
def Automan(folder, logfile, evecfile, evalfile, mode, domain, resnumber):
    ...
    if domain == 1:
        ...
        with open(folder+"/selected_eigens.txt", "w") as selected_eigenvectors:
            selected_eigenvectors.write("\nSELECTED EIGENS: %s\n" % selected)
    ...
    if not mode == 'b':
        ...
        selected_eigenvects = ProjectSelectedEigens(folder, resnumber, domain, evecs, sel_eig)
        PYMOL_Projection_b(mode, domain, sel_eig, selected_eigenvects, folder, resnumber)
        PYMOL_Projection(mode, domain, sel_eig, selected_eigenvects, folder, resnumber)
        PYMOL_Projection_b_T(mode, domain, sel_eig, selected_eigenvects, folder, resnumber)
...
def ProjectSelectedEigens(folder, resnumber, domain, eigenvectors, sel_eig):

    print("\nI- %s projecting eigenvectors on structure...\n" % str(date('%Y-%m-%d %H:%M:%S')))
    threshold = math.sqrt(1./float(resnumber))
    list_eigens_components = []

    if domain == 0:
        selected_component = np.zeros(resnumber)
        selected_component_T = np.zeros(resnumber)
        for i in range(0, resnumber):
            selected_component[i] = np.abs(eigenvectors[i, 0])
            if np.abs(eigenvectors[i, 0]) > threshold:
                selected_component_T[i] = np.abs(eigenvectors[i, 0])
        
        list_eigens_components = selected_component_T
        
        eigenvector_profile_b = ''
        eigenvector_profile_b_T = ''
        for i in range(0, selected_component.shape[0]):
            eigenvector_profile_b += "%d  %.6f\n" % (i, selected_component[i])
            eigenvector_profile_b_T += "%d  %.6f\n" % (i, selected_component_T[i])

        with open(folder + '/enedist_eigen_0.dat', 'w') as seleig:
            seleig.write("%s" % eigenvector_profile_b)
            
        with open(folder + '/enedist_eigen_0_T.dat', 'w') as seleigT:
            seleigT.write("%s" % eigenvector_profile_b_T)
        ...
        for index in sel_eig:
            ...
            selected_component_T = np.zeros(resnumber)
            for i in range(0, resnumber):
                selected_component[i] = np.abs(eigenvectors[i, index])
                if np.abs(eigenvectors[i, index]) > threshold:
                    selected_component_T[i] = np.abs(eigenvectors[i, index])    

            list_eigens_components.append(selected_component_T)
            ...
            with open(folder + '/enedist_eigen_%s.dat' % (index), 'w') as seleig:
                seleig.write("%s" % eigenvector_profile_b)

            with open(folder + '/enedist_eigen_%s_T.dat' % (index), 'w') as seleigT:
                seleigT.write("%s" % eigenvector_profile_b_T)

    return list_eigens_components

def PYMOL_Projection(mode, domain, sel_eig, eigenvectors, folder, resnumber):
    colors = ['marine','orange','forest','firebrick','brown','deeppurple','deepteal','olive','deepsalmon','smudge','slate','acquamarine','yelloworange','green','blue','sand','red','lightmagenta','lightblue','lightorange','palegreen']

    script = open(folder + '/selected_eigenvects_projection.py', 'w')
...
\n''')
    ...
    if mode == 's':
        load = "cmd.load(\'snapshot.AMBER.pdb\')"
        rename = "cmd.set_name(\'snapshot.AMBER\', \'template\')"
    else:
        load = "cmd.load(\'FRAME_1/frame_1.pdb\')"
        rename = "cmd.set_name(\'frame_1\', \'template\')"

    script.write(load+"\n"+rename)
    script.write("\ncmd.hide(\'lines\')\ncmd.show(\'cartoon\')\ncmd.set(\'cartoon_fancy_helices\')\ncmd.color(\'gray90\', \'all\')\n\n")

    if domain == 0:
        script.write("cmd.set_name(\'template\', \'eigen_0\') \n")
        script.write("eigenvector_0 = np.zeros(%s) \n" % (resnumber))
        script.write("with open(\'enedist_eigen_0_T.dat\', \'r\') as ev_file: \n")
        script.write("\tev_lines = ev_file.readlines() \n")
        script.write("for j, ev_line in enumerate(ev_lines): \n")
        script.write("\tev_words = ev_line.split() \n")
        script.write("\teigenvector_0[j] = float(ev_words[1]) \n")
        script.write("for i, value in enumerate(eigenvector_0):\n")
        script.write("\tif value > 0: \n")
        script.write("\t\tcmd.color(\'red\', \'eigen_0 and resi %s\' % (i + 1)) \n\n")

    if domain == 1:
        for i, eig in enumerate(sel_eig):
            script.write("cmd.copy(\'eigen_%s\', \'template\') \n" % (eig))
            script.write("eig = %s \n" % (eig))
            script.write("eigenvector_%s = np.zeros(%s) \n" % (eig, resnumber))
            script.write("with open(\'enedist_eigen_%s_T.dat\', \'r\') as ev_file: \n" % (eig))
            script.write("\tev_lines = ev_file.readlines() \n")
            script.write("for j, ev_line in enumerate(ev_lines): \n")
            script.write("\tev_words = ev_line.split() \n")
            script.write("\teigenvector_%s[j] = float(ev_words[1]) \n" % (eig))
            script.write("for i, value in enumerate(eigenvector_%s):\n" % (eig))
            script.write("\tif value > 0: \n")
            script.write("\t\tcmd.color(\'red\', \'eigen_%s and resi %s\' % (eig, i + 1)) \n\n\n")  
        script.write("cmd.delete(\'template\') \n")

    script.close()

def PYMOL_Projection_b(mode, domain, sel_eig, eigenvectors, folder, resnumber):
    colors = ['marine','orange','forest','firebrick','brown','deeppurple','deepteal','olive','deepsalmon','smudge','slate','acquamarine','yelloworange','green','blue','sand','red','lightmagenta','lightblue','lightorange','palegreen']

    script = open(folder + '/selected_eigenvects_projection_bfactor.py', 'w')
...
\n''')
    ...
    script.write(load+"\n"+rename)
    script.write("\ncmd.hide(\'lines\')\ncmd.show(\'cartoon\')\ncmd.set(\'cartoon_fancy_helices\')\ncmd.color(\'gray60\', \'all\')\n\n")

    if domain == 0:
        script.write("cmd.set_name(\'template\', \'eigen_0\') \n")
        script.write("eigenvector_0 = np.zeros(%s) \n" % (resnumber))
        script.write("with open(\'enedist_eigen_0.dat\', \'r\') as ev_file: \n")
        script.write("\tev_lines = ev_file.readlines() \n")
        script.write("for j, ev_line in enumerate(ev_lines): \n")
        script.write("\tev_words = ev_line.split() \n")
        script.write("\teigenvector_0[j] = float(ev_words[1]) \n")
        script.write("minval = eigenvector_0.min() * 1000 \n")
        script.write("maxval = eigenvector_0.max() * 1000 \n")
        script.write("stored.newB = [] \n")
        script.write("for value in eigenvector_0:\n")
        script.write("\tstored.newB.append( float(value)*1000 ) \n")
        script.write("cmd.alter(\'eigen_0\', \'b=0.0\')\n")
        script.write("cmd.alter(\'eigen_0 and n. CA\', \'b=stored.newB.pop(0)\') \n")
        script.write("cmd.spectrum(\'b\', \'white_red\', \'eigen_0 and name CA\', minimum=minval, maximum=maxval) \n\n")
        script.write("cmd.copy(\'eigen_0_putty\', \'eigen_0\') \n")

        script.write("cmd.cartoon(\'putty\', \'eigen_0_putty\') \n")
        script.write("cmd.set(\'cartoon_putty_scale_min\', minval, \'eigen_0_putty\') \n")
        script.write("cmd.set(\'cartoon_putty_scale_max\', maxval, \'eigen_0_putty\') \n")
        script.write("cmd.set(\'cartoon_putty_transform\', 4, \'eigen_0_putty\') \n")
        script.write("cmd.set(\'cartoon_putty_radius\', 0.2, \'eigen_0_putty\') \n\n")
    ...
    if domain == 1:
        ...
        eigenvector = []
        for i, eig in enumerate(sel_eig):
            with open("enedist_eigen_%s.dat" % (eig), 'r') as evm_file:
                evm_lines = evm_file.readlines()
            for j, evm_line in enumerate(evm_lines):
                evm_words = evm_line.split()
                eigenvector.append(float(evm_words[1]))
            mintemp = np.min(eigenvector)
            maxtemp = np.max(eigenvector)
            if mintemp < minv:
                minv = mintemp
            if maxtemp > maxv:
                maxv = maxtemp
        ...
        for i, eig in enumerate(sel_eig):
            script.write("cmd.copy(\'eigen_%s\', \'template\') \n" % (eig))
            script.write("eigenvector_%s = np.zeros(%s) \n" % (eig, resnumber))
            script.write("with open(\'enedist_eigen_%s.dat\', \'r\') as ev_file: \n" % (eig))
            script.write("\tev_lines = ev_file.readlines() \n")
            script.write("for j, ev_line in enumerate(ev_lines): \n")
            script.write("\tev_words = ev_line.split() \n")
            script.write("\teigenvector_%s[j] = float(ev_words[1]) \n" % (eig))
            script.write("minval = eigenvector_%s.min() * 1000 \n" % (eig))
            script.write("maxval = eigenvector_%s.max() * 1000 \n" % (eig))
            script.write("stored.newB = [] \n")
            script.write("for value in eigenvector_%s:\n" % (eig))
            script.write("\tstored.newB.append( float(value) * 1000 ) \n")
            script.write("cmd.alter(\'eigen_%s\', \'b=0.0\')\n" % (eig))
            script.write("cmd.alter(\'eigen_%s and n. CA\', \'b=stored.newB.pop(0)\') \n" % (eig))
            script.write("cmd.spectrum(\'b\', \'white red\', \'eigen_%s and name CA\', minimum=minval, maximum=maxval) \n" % (eig))
            script.write("cmd.copy(\'eigen_%s_putty\', \'eigen_%s\') \n" % (eig, eig))

            script.write("cmd.cartoon(\'putty\', \'eigen_%s_putty\') \n" % (eig))
            script.write("cmd.set(\'cartoon_putty_scale_min\', minval, \'eigen_%s_putty\') \n" % (eig))
            script.write("cmd.set(\'cartoon_putty_scale_max\', maxval, \'eigen_%s_putty\') \n" % (eig))
            script.write("cmd.set(\'cartoon_putty_transform\', 4, \'eigen_%s_putty\') \n" % (eig))
            script.write("cmd.set(\'cartoon_putty_radius\', 0.2, \'eigen_%s_putty\') \n\n" % (eig))

        script.write("cmd.delete(\'template\') \n")

    script.close()

def PYMOL_Projection_b_T(mode, domain, sel_eig, eigenvectors, folder, resnumber):

    colors = ['marine','orange','forest','firebrick','brown','deeppurple','deepteal','olive','deepsalmon','smudge','slate','acquamarine','yelloworange','green','blue','sand','red','lightmagenta','lightblue','lightorange','palegreen']

    script = open(folder + '/selected_eigenvects_projection_bfactor_threshold.py', 'w')
...
\n''')
    ...
    if domain == 0:
        ...
        script.write("\tev_words = ev_line.split() \n")
        script.write("\teigenvector_0[j] = float(ev_words[1]) \n")
        script.write("minval = 0 \n")
        script.write("maxval = eigenvector_0.max() \n")
        script.write("stored.newB = [] \n")
        script.write("for value in eigenvector_0:\n")
        script.write("\tstored.newB.append( float(value) ) \n")
        script.write("cmd.alter(\'eigen_0\', \'b=0.0\')\n")
        script.write("cmd.alter(\'eigen_0 and n. CA\', \'b=stored.newB.pop(0)\') \n")
        script.write("cmd.spectrum(\'b\', \'yellow red\', \'eigen_0 and name CA\', minimum=minval, maximum=maxval) \n")
        script.write("for k, value in enumerate(eigenvector_0):\n")
        script.write("\tresi = k + 1 \n")
        script.write("\tif value == 0: \n")
        script.write("\t\tcmd.color(\'white\', \'eigen_0 and resi %s\' % (resi)) \n\n")

    if domain == 1:
        for i, eig in enumerate(sel_eig):
            script.write("current_eig = %s \n" % (eig))
            script.write("cmd.copy(\'eigen_%s\', \'template\') \n" % (eig))
            script.write("eigenvector_%s = np.zeros(%s) \n" % (eig, resnumber))
            script.write("with open(\'enedist_eigen_%s_T.dat\', \'r\') as ev_file: \n" % (eig))
            script.write("\tev_lines = ev_file.readlines() \n")
            ...
            script.write("\teigenvector_%s[j] = float(ev_words[1]) \n" % (eig))
            script.write("minval = 0 \n")
            script.write("maxval = eigenvector_%s.max() \n" % (eig))
            script.write("stored.newB = [] \n")
            script.write("for value in eigenvector_%s:\n" % (eig))
            script.write("\tstored.newB.append( float(value) ) \n")
            script.write("cmd.alter(\'eigen_%s\', \'b=0.0\')\n" % (eig))
            script.write("cmd.alter(\'eigen_%s and n. CA\', \'b=stored.newB.pop(0)\') \n" % (eig))
            script.write("cmd.spectrum(\'b\', \'yellow red\', \'eigen_%s and name CA\', minimum=minval, maximum=maxval) \n\n\n" % (eig))
            script.write("for k, value in enumerate(eigenvector_%s):\n" % (eig))
            script.write("\tresi = k + 1 \n")
            script.write("\tif value == 0: \n")
            script.write("\t\tcmd.color(\'white\', \'eigen_%s and resi %s\' % (current_eig, resi)) \n\n")
